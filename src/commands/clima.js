...
export default {
  name: "clima",
  description: "Obtén el clima actual de tu ciudad o de tu pais rapidamente.",
  alias: ["tiempo", "tiemp"],
  use: "!clima 'ciudad o país'",
  ...
  run: async (socket, msg, args) => {
    ...
    if (!city) {
      socket.sendMessage(msg.messages[0]?.key?.remoteJid, {
        text: `*Escriba el nombre de pais o ciudad que desees*`,
      });
    ...
    try {
      ...
      socket.sendMessage(msg.messages[0]?.key?.remoteJid, {
        text: `*No se han encontrado resultados, Revise si haya escrito correctamente su país o ciudad👻*`,
      });
    }
  },
};
