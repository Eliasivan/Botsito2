    ...
    try {
      ...
      if (!url) {
        socket.sendMessage(msg.messages[0]?.key.remoteJid, {
          text: "Ingrese la URl del vídeo de TikTok que deseas descargar sin marca de agua.",
        });
      ...
      if (!regexp.test(url)) {
        socket.sendMessage(msg.messages[0]?.key.remoteJid, {
          text: "URL inválida, prueba con otra.",
        });
      ...
      if (status !== "success") {
        socket.sendMessage(msg.messages[0]?.key.remoteJid, {
          text: "Ha ocurrido un error, vuelve a intentarlo nuevamente.",
        });
        ...
        if (result.type === "image") {
          ...
          if (media === "limit exceeded") {
            await socket.sendMessage(msg.messages[0]?.key?.remoteJid, {
              text: "No pude enviar el video ya que este es demasiado pesado para mi.",
            });
